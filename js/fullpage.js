(function() {
	...
	Fullpage.prototype = {
		...
		disindex: function(_this) {
			...
			_this.newconfig.callback[_this.newconfig.index]();
		},
		//进入指定页 fn为回调函数
...
(function(){
	...
	Nav = function (nav) {
		var _this = this;
		this.main = document.getElementById(nav);
	};
	Nav.prototype={
		reheght:function(){
			startMove(this.main,{height:100}, null, 10);
			startMove(this.main.children[0],{height:100},null,10);
			for(var i= 0,length=this.main.children[0].children.length;i<length;i++){
				startMove(this.main.children[0].children[i],{height:100},null,10);
				this.main.children[0].children[i].className="lists listsh1";
				this.main.children[0].children[i].children[0].className="alists font1";
			}
			startMove(this.main.children[1],{height:100},null,10);
			this.main.children[1].className="inline1";
			this.main.children[1].children[0].className="font1";
		},
		recolor1:function(){
			this.main.className="nav color1";
		},
		realth:function(){
			startMove(this.main,{height:70}, null, 10);
			startMove(this.main.children[0],{height:70},null,10);
			for(var i= 0,length=this.main.children[0].children.length;i<length;i++){
				startMove(this.main.children[0].children[i],{height:70},null,10);
				this.main.children[0].children[i].className="lists";
				this.main.children[0].children[i].children[0].className="alists";
			}
			startMove(this.main.children[1],{height:70},null,10);
			this.main.children[1].className=" ";
			this.main.children[1].children[0].className=" ";
			this.main.className="nav";
		},
		recolor2:function(){
			this.main.className="nav color2";
			for(var i= 0,length=this.main.children[0].children.length;i<length;i++){
				this.main.children[0].children[i].children[0].className="alists font1 fontcolor2"
			}
			this.main.children[1].children[0].className="font1 fontcolor2";
		},
		recolor3: function () {
			this.main.className="nav color3";
			for(var i= 0,length=this.main.children[0].children.length;i<length;i++){
				this.main.children[0].children[i].children[0].className="alists font1 fontcolor3"
			}
			this.main.children[1].children[0].className="font1 fontcolor3";
		}
	};
	window.Nav=Nav;
})(window);
(function(){
	...
	jsonp.prototype = {
		...
		setjsonp: function () {
			this.js.src =this.url+ "?appkey=135bb18481932b78&callback=" +this.name;
